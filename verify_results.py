...
# =============================================================================
# CONFIGURATION — UNIFIED DOI
# =============================================================================
DOI = "10.5281/zenodo.19440498"  # ← Единый источник истины
...
REF_VALUES = {
    'Lx_median': 28.57, 'Lx_err_plus': 0.73, 'Lx_err_minus': 0.87,
    'H0_mean': 67.55, 'H0_std': 1.77,
    'dchi2': -5.33, 'dBIC': 2.49, 'corr_Lx_H0': 0.258,
    'SH0ES_H0': 73.04, 'SH0ES_err': 1.04,
    'N_eff': 17633, 'survival_rate': 0.27,
    'chi2_it3': 2484.12, 'chi2_lcdm': 2489.45,
}
...
def load_chains(filepath, description="chains"):
    """Load MCMC chains from .npy or .pkl with auto-detection"""
    filepath = Path(filepath)
    
    if not filepath.exists():
        pkl_alternatives = []
        if "chains_IT3" in str(filepath):
            pkl_alternatives = [Path("samples_it3.pkl"), Path("data/samples_it3.pkl")]
        elif "chains_LCDM" in str(filepath):
            pkl_alternatives = [Path("samples_lcdm.pkl"), Path("data/samples_lcdm.pkl")]
        ...
        raise FileNotFoundError(
            ...
            f"💡 Run 'python convert_chains.py' first, or specify correct path with --chains"
        )
...
def compute_omega_m(chains, obh2_idx=OBH2_IDX, och2_idx=OCH2_IDX):
    ...
    return (chains[:, obh2_idx] + chains[:, och2_idx]) / (h ** 2)
...
def importance_sampling_weights(chains, H0_target=68.5, H0_err=1.5, Om_target=0.28, Om_err=0.015):
...
def test_parameter_constraints(chains):
    ...
    if chains.shape[1] <= LX_IDX:
        print(f"❌ Chains have {chains.shape[1]} params, expected ≥ {LX_IDX+1}")
        return False
...
def test_correlation_Lx_H0(chains):
    ...
    print("-" * 50)
    if chains.shape[1] <= LX_IDX:
        print("❌ Cannot compute correlation: Lx not found")
        return False
    Lx = chains[:, LX_IDX]
    H0 = chains[:, H0_IDX]
...
def generate_corner_plot(chains, output_path="figures/verify_corner.png"):
    ...
    fig = corner.corner(chains[:, plot_idx], labels=labels, quantiles=[0.16, 0.5, 0.84], show_titles=True, title_fmt=".2f")
...
def generate_correlation_plot(chains, output_path="figures/verify_correlation.png"):
    ...
    plt.axhline(REF_VALUES['SH0ES_H0'], color='green', linestyle='--', label=f"SH0ES: {REF_VALUES['SH0ES_H0']}±{REF_VALUES['SH0ES_err']}")
    ...
    plt.plot(x_fit, slope * x_fit + intercept, 'w--', label=f"Fit: r={r_val:.3f}")
...
def main():
    parser = argparse.ArgumentParser(description="Verify IT3 topology model results",
        epilog="Examples:\n  python verify_results.py\n  python verify_results.py --chains samples_it3.pkl",
    ...
    results = {
        'constraints': test_parameter_constraints(chains_it3),
        'model_comp': test_model_comparison(chains_it3, chains_lcdm),
        'correlation': test_correlation_Lx_H0(chains_it3),
        'importance': test_importance_sampling(chains_it3),
        'hubble': test_hubble_tension(chains_it3),
    }
    ...
    with open(report_path, 'w') as f:
        f.write(f"IT3 Verification Report\nTimestamp: {np.datetime64('now')}\nDOI: {DOI}\nOverall: {'PASS' if all_pass else 'FAIL'}\n\n")
        for test, passed in results.items():
